...
2. Datalake
3. Data Service (Currently CAI, CDW, CDE & CDF)
4. `Keycloak` Server
5. Users
...
The same CAII restriction is enforced in the Jenkins parameter validation stage and in the container `configfile` check at runtime.
== Technology Stack
...
3.  When `Keycloak` Server is provisioned using this automation, the workshop owner can create the required number of workshop users with Keycloak (SSO) based access to the workshop environment (depending on the type of workshop e.g. CAI, CDW, or CDE). The Keycloak server will be HTTPS (TLS/SSL Secured) with LetsEncrypt CA.
...
8.  Optional activation of CDW, CDE, and CAI data services and provisioning of the required resources e.g. Virtual Service (CDE), Virtual Warehouses (CDW), Virtual Workspace (CAI), etc.
...
12. Optional deployment of GPU instances with CAI workspaces.
