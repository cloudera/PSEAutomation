...
### Fixed
- AWS destroy: call `cdp environments delete-environment` before CDP quickstart Terraform (`HOL_CDP_DELETE_ENV_BEFORE_TF`, default on); expand `hol_aws_trace_vpc_destroy_dependencies` (CDP env, ELB, CFN stacks, EC2, all ENIs) when leaf EIP/NAT scans are insufficient.
- AWS/Azure destroy: remove default CDP list-API poll in `destroy_cdp` (disable playbooks already wait); one-shot strict check only; skip when CDP env and workshop VPC are already absent; optional `HOL_CDP_DS_DESTROY_WAIT_SEC` (default 0).
...
- AWS destroy: `hol_terraform_timed` runs CDP Terraform destroy under `timeout` via a sourced bash subshell (fixes `timeout: hol_terraform: No such file or directory`). AWS `destroy_keycloak` skips when Terraform dir/state is missing (manual Keycloak cleanup).
...
- AWS destroy (VPC DependencyViolation): wait for CDP data services to leave list APIs before CDP Terraform destroy; pre-destroy VPC cleanup (NAT gateway poll, workshop-tagged unassociated EIP release, ENI/instance warnings); retry CDP Terraform on IGW/subnet mapped-public-address errors; align teardown order with Azure (Keycloak → `destroy_aws_enhancements` → CDP Terraform). Document manual AWS cleanup in `docs/oncloud/aws-provisioner.adoc`.
