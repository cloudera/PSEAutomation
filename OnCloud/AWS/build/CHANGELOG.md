...
### Changed
- Data services now provision and tear down in parallel, with live per-service logs, progress heartbeats, retry status, and clear completion summaries.
- AWS teardown deletes environment Data Hubs by default before infrastructure destruction. Set `DELETE_DATAHUBS=false` to retain them.
- CDP environment role assignment is performed only at the required lifecycle stages and retrieves existing roles once per principal.
- Jenkins pipelines support consistent refresh-and-exit behavior through `REFRESH_JENKINSFILE`.
- AWS Docker image references now use the cloud-specific `clouderapartners/cdp-public-cloud-hol-provisioner-aws` repository, matching the Azure image naming convention.

### Added
- Colorized Jenkins, Terraform, role-assignment, and Ansible output, with standard color opt-outs.
- Explicit success messages for CDW, CDE, and CDF provisioning and teardown.
- Standalone provisioner support for assigning environment roles to the active CDP API caller and an optional build user.

### Fixed
- Data Hub cleanup is idempotent when the CDP environment is already absent, while genuine API failures still stop teardown.
- AWS destroy now reports VPC blockers, bounds and retries Terraform teardown, handles dependency ordering, and propagates failures instead of reporting false success.
- Parallel service batches wait for all selected playbooks, aggregate failures, and return a non-zero status to Jenkins.
- CDF provisioning and teardown are idempotent across healthy, in-progress, absent, and failed states, with improved authorization retries and correct default instance-type handling.
- CDE defaults, Spark-version resolution, active-tier sizing, autoscaling, and Python 3.12 compatibility are corrected.
- CDW teardown verification now consistently resolves the workshop environment name.
- Keycloak user synchronization correctly waits for an already-running operation, and IDP playbooks use the same streaming/error handling as data-service playbooks.
- Local Jenkins image builds consistently use the selected image tag and invalidate HoL copy layers when the source revision changes.
- Selected data services, including CDF and legacy `CML`/CAI selections, are reliably passed from PollSCM through parallel provisioning.
